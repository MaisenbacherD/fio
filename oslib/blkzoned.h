...
extern int blkzoned_finish_zones(struct thread_data *td, struct fio_file *f,
				uint64_t offset, uint64_t length);
#else
/*
...
static inline int blkzoned_finish_zones(struct thread_data *td, struct fio_file *f,
				    uint64_t offset, uint64_t length)
{
	return -EIO;
}
